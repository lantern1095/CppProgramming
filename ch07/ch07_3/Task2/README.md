...
- 전위 `++` 연산자를 오버로딩하는 프렌드 함수 선언

`friend Complex operator-- (Complex& op, int x);`

- 후위 `--` 연산자를 오버로딩하는 프렌드 함수 선언
...
- 매개변수로 받은 `op` 객체의 `real` 값을 1 증가시킴

`op.img++;`

- 매개변수로 받은 `op` 객체의 `img` 값을 1 증가시킴

`return op;`

- 연산이 완료된 `op` 객체 자신을 참조 형태로 반환한다.
...
- 매개변수로 받은 `op` 객체의 `real` 값을 1 감소시킴

`op.img = op.img - 2;`

- 매개변수로 받은 `op` 객체의 `img` 값을 2 감소시킴
...
## 실행결과
<img width="1107" height="190" alt="image" src="https://github.com/user-attachments/assets/c17ffe92-07b7-4f2a-a416-21e1c6c8e883" />
